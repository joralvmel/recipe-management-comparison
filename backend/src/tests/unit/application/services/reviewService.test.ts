...
describe('ReviewService', () => {
  ...
  describe('addReview', () => {
    it('should add a review successfully', async () => {
      ...
      reviewRepository.addReview.mockResolvedValue(new Review('user1', 'user1', 'recipe1', 5, 'Great recipe!'));

      const result = await reviewService.addReview('user1', 'user1', 'recipe1', 5, 'Great recipe!');

      expect(result).toEqual(new Review('user1', 'user1', 'recipe1', 5, 'Great recipe!'));
      expect(reviewRepository.addReview).toHaveBeenCalledWith(new Review('user1', 'user1', 'recipe1', 5, 'Great recipe!'));
    });

    it('should throw ResourceAlreadyExistsError if review already exists', async () => {
      reviewRepository.getReviewsByRecipe.mockResolvedValue([new Review('user1', 'user1', 'recipe1', 5, 'Great recipe!')]);

      await expect(reviewService.addReview('user1', 'user1', 'recipe1', 5, 'Great recipe!')).rejects.toThrow(
  ...
  describe('editReview', () => {
    it('should edit a review successfully', async () => {
      const existingReview = new Review('user1', 'user1', 'recipe1', 5, 'Great recipe!');
      reviewRepository.getReviewById.mockResolvedValue(existingReview);
      reviewRepository.editReview.mockResolvedValue(new Review('user1', 'user1', 'recipe1', 4, 'Good recipe!'));

      const result = await reviewService.editReview('user1', 'review1', { rating: 4, content: 'Good recipe!' });

      expect(result).toEqual(new Review('user1', 'user1', 'recipe1', 4, 'Good recipe!'));
    ...
    it('should throw ForbiddenError if user is not authorized to edit the review', async () => {
      const existingReview = new Review('user2', 'user2', 'recipe1', 5, 'Great recipe!');
      reviewRepository.getReviewById.mockResolvedValue(existingReview);
  ...
  describe('getReviewsByRecipe', () => {
    it('should return reviews for a recipe', async () => {
      const reviews = [new Review('user1', 'user1', 'recipe1', 5, 'Great recipe!'), new Review('user2', 'user2', 'recipe1', 4, 'Good recipe!')];
